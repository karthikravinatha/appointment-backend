...
CREATE FUNCTION fGetCollectionByZone(PZone_ID INT, PType INT)
  ...
  BEGIN
    RETURN (SELECT Round(sum(f.Inward_Quantity), 2) AS Quantity2
            ...
            WHERE
              fGetFinacialYearByDate(k.Bill_Date) = fGetFinacialYearByDate(CURDATE())
              AND
              CASE WHEN PType = 3
                THEN
                  MONTH(k.Bill_Date) = MONTH(CURDATE())
              ELSE
                1 = 1
              END
              AND
              CASE WHEN PType = 1
                THEN
                  k.Bill_Date = subdate(CURDATE(), 1)
              ELSE
                1 = 1
              END AND
              CASE WHEN PType = 2
                THEN
                  WEEK(DATE(k.Bill_Date)) = WEEK(CURDATE())
              ELSE
                1 = 1
              END
...
DROP PROCEDURE IF EXISTS fGetStateNameByEntity;
