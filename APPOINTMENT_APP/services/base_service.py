...
from APPOINTMENT_APP.utils.cache_manager.cachable import ICachable
from APPOINTMENT_APP.utils.cache_manager.cache_manager import RedisCacheManager
from APPOINTMENT_APP.repository.transaction_repository import TransactionRepository
from APPOINTMENT_APP.repository.normal_repository import NormalRepository
from APPOINTMENT_APP.utils.exception_handling.exception import AppointmentException, DatabaseException, CacheException, \
    ValidationException
from APPOINTMENT.settings import base
...
class BaseService:
    ...
    def add(self, object):
        ...
        try:
            ...
            self.rollback()
            raise AppointmentException(ex.error_code, ex.error_message, ex.error_object)
            ...
            self.rollback()
            raise AppointmentException(self.general_error_code, str(ex1), None)
    ...
    def get_common(self, sp_name, params: ParamsObject, is_cacheable=False, cache_key_prefix=""):
        ...
        try:
            ...
            if base.DEBUG_ENABLED:
                LoggingHelper().log_error(self.__class__, "", str(ex))
            self.rollback()
            raise AppointmentException(self.general_error_code, str(ex), None)
        finally:
            self.close_cursor()
