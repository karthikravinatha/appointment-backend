...
from APPOINTMENT_APP.utils.constants.constants import HttpMethodType, DataTypes, ErrorCodes
from enum import Enum

from APPOINTMENT_APP.utils.exception_handling.exception import AppointmentException
...
class RequestHelper:
    ...
    def get_session_value(self, request, key, session_key, datatype, default_value):
        ...
        if self.token_payload is None:
            self.token_payload = JWTManager.decode_token(
                request.META['HTTP_AUTHORISATION'].replace("Bearer ", ''))
    ...
    def call_ext_api(self, request: HttpRequest, host, is_token_needed=True):
        ...
        try:
            ...
            raise AppointmentException(ErrorCodes.GENERAL_ERROR, str(ex), None)
