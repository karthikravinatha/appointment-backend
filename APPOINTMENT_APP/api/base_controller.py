...
from APPOINTMENT_APP.api.transaction_log import TransactionLog
from APPOINTMENT_APP.utils.helpers.logging_helper import LoggingHelper
from APPOINTMENT_APP.utils.helpers.request_helper import RequestHelper, ResponseObject
from APPOINTMENT_APP.utils.helpers.session_helper import SessionHelper, JWTManager
import json
...
class BaseController:
    ...
    def __add_log(self, ext_params, user_id, transaction_id, transaction_type, transaction_action, data_object):
        try:
            transaction_log: TransactionLog = TransactionLog(ext_params)
            transaction_log.add(user_id, transaction_id, transaction_type, transaction_action, data_object)
        except Exception as ex:
            pass

    def add_log(self, ext_params, user_id, transaction_id, transaction_type, transaction_action, data_object):
        try:
            t = threading.Thread(target=self.__add_log,
                                 args=(ext_params, user_id, transaction_id, transaction_type, transaction_action,
                                       data_object))
            t.start()
            return True
        except Exception as ex:
